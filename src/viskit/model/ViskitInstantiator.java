...
public abstract class ViskitInstantiator
{
	...
	/**
	 * Create initial placeholder structures for complex types so that Object Inspector can finish the job later
	 * @param reflectionExecutables an array of constructor methods created by .class reflection
	 * @return 
	 */
    public static Vector<Object> buildDummyInstantiatorsFromReflection(Executable[] reflectionExecutables) 
	{
		...
		for (Executable reflectionExecutable : reflectionExecutables)
		{
			...
			for (Class<?> reflectionParameterClass : reflectionParameterClasses) // skips zero-parameter constructor
			{
				...
				{
					viskitInstantiatorVector.add(new ViskitInstantiator.Array(className, new ArrayList<>())); // TODO how to add name? probably deferred
				}
////////				else if (className.equals("simkit.random.RandomVariate"))
////////					// reflectionParameterClass.???) // TODO trying to construct factory type...
////////					// but likely better to do later, by user via Object Inspector (tho that requires knowledge) or by GUI helpfulness
////////				{
////////					// TODO can we create a pseudo list of arguments for end user to customize??
////////					viskitInstantiatorVector.add(new ViskitInstantiator.Construct(className, null)); 
////////				}
				else 	
				{
			...
			if (viskitInstantiatorVector.size() > 0) // reflection found parameters during preceding iteration; 
				// only return upon first discovery to avoid multiple constructor tabs
			{
        ...
        public Construct(List<Object> assemblyParametersList, String typeName)
		{
            ...
            if (viskit.ViskitStatics.debug) 
			{
				...
				if ((assemblyParametersList == null) || assemblyParametersList.isEmpty())
					   noParameters = ": none";
				else   noParameters = ": " + assemblyParametersList.size();
...
//				((ViskitInstantiator) viskitParametersFactoryList.get(index)).setDescription(assemblyParameterDescription);
        ...
        public Construct(String typeName, List<Object> argumentsList)
		{
            super(typeName);
            setParametersFactoryList(argumentsList); // simple assignment. no arguments are provided unless they were previously initialized.
            findArgumentNames(typeName, argumentsList);
        }
         ...
         * @param typeName
         * @param arguments List of ViskitInstantiators (if already defined)
         * @return true if argument names are found, either in the provided arguments list or via reflection
         */
        private boolean findArgumentNames(String typeName, List<Object> arguments)
		{
            if (arguments == null) // must proceed with reflection to find parameters list
			{
                setParametersFactoryList(getDefaultParametersFromReflection(typeName));
                arguments = getParameterTypesList(); // factory, simple type or perhaps an object constructor
            }
        ...
        public int indexOfArgumentNames(String typeName, List<Object> arguments)
		{
            ...
            if (viskit.ViskitStatics.debug)
			{
                LOG.info("arguments length " + arguments.size() + ", indexOfArgumentNames for " + typeName + " has parameterArrayList.length " + parameterArrayList.length);
            }
            for (List<Object> parameter : parameterArrayList)
			{
                if (viskit.ViskitStatics.debug)
				{
                    LOG.info("indexOfArgumentNames: parameter.size() " + parameter.size());
                }
...
//                                    + ((ViskitInstantiator) arguments.get(j)).getTypeName());
			...
			{
                LOG.info("Resolving " + typeName + " " + parameterArrayList[constructorIndex].getClass() + " at index " + constructorIndex);
            }
        ...
        private List<Object> getDefaultParametersFromReflection(String typeName)
		{
            ...
            if (inputClass != null)
			{
                ...
                if (reflectionConstructors != null && reflectionConstructors.length > 0)
				{
                    ...
                    return ViskitInstantiator.buildDummyInstantiatorsFromReflection(reflectionConstructors);
                }
            }
            return new Vector<>(); // null
        }
		
		/** factory, simple type or perhaps an object constructor */

        public List<Object> getParameterTypesList()
		{
        ...
        public ViskitInstantiator.Construct vcopy()
		{
			...
			{
				if (viskitObject instanceof ViskitInstantiator) // check types before creation
				{
					if (((ViskitInstantiator) viskitObject).type.equals("simkit.random.RandomVariate"))
					{
						// create parent element
						String            typeName = ((ViskitInstantiator) viskitObject).getTypeName(); // type is duplicated in contained TerminalParameter
						String     factoryClassName = typeName + "Factory";
						String           methodName = "getInstance";
						List<Object> parametersList = new ArrayList<Object>();
						             parametersList.add (((ViskitInstantiator)viskitObject).vcopy()); // pass along original, will become TerminalParameter
						ViskitInstantiator.Factory newViskitFactory = new ViskitInstantiator.Factory(
							typeName,
							factoryClassName,
							methodName,
							parametersList
						);
						objectVector.add(newViskitFactory); // coerce to factory type, containing original terminal parameter
					}
					else 
					{
    ...
    public static class Factory extends ViskitInstantiator
	{
        ...
        public boolean isValid()
		{
            ...
            if (typeName == null   || methodName == null ||
                typeName.isEmpty() || methodName.isEmpty())
			{
                return false;
            }
